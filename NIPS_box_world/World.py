...
class World(object):
    ...
    def __init__(self,number=1):
        ...
        self.boxes = self.get_boxes(7)
        self.trucks = self.get_trucks(3)
...
def train_regression_model():
    '''trains regression model
       on training data
    '''
    call_process('java -jar BoostSRL-v1-0.jar -reg -l -train train -target value -trees '+str(trees))
...
def get_projected_values():
    ...
    '''
    call_process('java -jar BoostSRL-v1-0.jar -i -reg -test test -target value -model train/models -trees '+str(trees))
...
def write_BE(BEs):
    ...
    global trees
    if trees > 1:
        call_process('rm BEGB.txt')
        with open("BEGB.txt","a") as f:
            for BE in BEs:
                f.write(str(BE)+"\n")
    else:
        call_process('rm BERRT.txt')
        with open("BERRT.txt","a") as f:
            for BE in BEs:
                f.write(str(BE)+"\n")
...
def main():
    ...
    global Transfer
    trees = int(argv[1])
    number_of_trajectories = int(argv[2])
    if len(argv)>3 and argv[3].lower()=="transfer":
        Transfer = True
    number_of_trajectories = burn_in_time+number_of_trajectories
    BEs = []
    for trajectory in range(number_of_trajectories):
        call_process('echo '+str(trajectory)+' >> output.txt')
    ...
    write_BE(BEs)
    if 'BEGB.txt' in listdir(".") and 'BERRT.txt' in listdir("."):
        call_process('python plot_BEs.py')
